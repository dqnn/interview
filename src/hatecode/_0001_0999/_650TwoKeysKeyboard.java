...
9/3=3 Copy 3 'A's 3 times (3),include one copy
3/3=1 Copy 1'A's 3 times (3)
Answer: 2+3+3= 8
*/
    //O(sqrt(N))/O(1)
public int minSteps_Best(int n) {
    int res = 0;
    for(int i=2;i<=n;i++){
        while(n%i == 0){
            res+= i;
    ...
    //interview friendly, 
    //so given a n, initially we have a "A", each move will be "COPY" or "PASTE", n=3, it would be 3. 
    //so dp[i] = dp[j] + (i/j); if i % j == 0, j = i-1,,....2
