...
Output: [9,7,8]
*/
    //so the problem is to say given a string s, we want to parition the string int several 
    //substring so each substring can have all chars which would not disappear in other partitions
    //and as many as possible
    
    //so we use sliding window, and we rememebr each char last character, everytime for each char
